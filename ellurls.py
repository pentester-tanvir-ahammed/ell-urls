...
def loader_animation(message="Processing..."):
    ...
    while not stop_loader:
        ...
        time.sleep(0.1)
    print("\r" + " " * len(message) + "\r", end="")  # Clear line
...
def print_ascii_art():
    ...
    for line in ascii_art_lines:
        print(line)
        time.sleep(0.2)
    animated_print("Most Powerful URLS Gathering Tool", delay=0.05)
...
# Fetch URLs from Wayback Machine (raw list)
def fetch_urls(target):
    ...
    try:
        with requests.get(archive_url, stream=True, timeout=60) as response:
            ...
            urls = [line for line in response.iter_lines(decode_unicode=True) if line]
            print(colored(f"\nFetched {len(urls)} URLs from archive.", "green"))
    except requests.exceptions.RequestException as e:
        print(colored(f"\nError fetching Wayback URLs: {e}", "red"))
        urls = []
    finally:
        stop_loader = True
        loader_thread.join()
...
def process_domain(target):
    ...
    file_path = os.path.join(folder, f"{target}_urls.txt")
    with open(file_path, 'w') as f:
        f.write("\n".join(urls))
    print(colored(f"All fetched URLs saved to: {file_path}", "green"))
    return urls
...
if __name__ == "__main__":
    ...
    for target in domains:
        ...
        urls = process_domain(target)
        enhanced = run_enhanced_url_gathering(target)
        all_unique_urls.update(urls)
        all_unique_urls.update(enhanced)
